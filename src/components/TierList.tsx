...
// Default S–F tiers with your chosen colors
const defaultTiers: Tier[] = [
  { id: "tier-1", label: "S", color: "bg-red-700" },
  { id: "tier-2", label: "A", color: "bg-orange-600" },
  { id: "tier-3", label: "B", color: "bg-yellow-500" },
  { id: "tier-4", label: "C", color: "bg-green-700" },
  { id: "tier-5", label: "D", color: "bg-blue-700" },
  { id: "tier-6", label: "F", color: "bg-gray-700" },
];

// 🎨 Palette of possible colors for *new* tiers
const tierColors = [
  "bg-red-700",
  "bg-orange-600",
  "bg-yellow-500",
...
export default function TierList({ teams }: Props) {
  ...
  // Add new tier with random color
  const addTier = () => {
    const newId = `tier-${tierNames.length + 1}`;
    const randomColor =
      tierColors[Math.floor(Math.random() * tierColors.length)];
  ...
  // Delete a tier → move its teams to Unassigned
          ...
          >
            {(provided) => (
              ...
              >
                ...
                {/* Tier Teams */}
                <div className="flex flex-wrap gap-2 flex-1 bg-gray-800 min-h-[96px] p-3 relative">
